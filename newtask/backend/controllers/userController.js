...
const getForm = async (req, res) => {
  try {
    const db = await form.find();
    console.log(db);
    if (db.length) {
      return res.status(200).send({ message: "form fetched successfully", db });
    } else {
      return res.status(404).send({ message: "No data fund in the DB" });
    }
  } catch (error) {
...
const updateForm = async (req, res) => {
  try {
    ...
    const updatedUser = await form.findByIdAndUpdate(userId, updatedUserData, {
      new: true,
    });
